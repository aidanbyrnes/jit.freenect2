...
void max_jit_freenect2_free(t_max_jit_freenect2 *x)
{
    if (x->qelem) qelem_free(x->qelem); // Free the qelem
    max_jit_mop_free(x);
...
void max_jit_freenect2_setup_callback(t_max_jit_freenect2 *x)
{
    ...
    if (jit_ob) {
        ...
        if (kinect) {
            ...
            post("jit.freenect2: Frame callback enabled. Output will be automatic.");
        }
    }
}
...
void max_jit_freenect2_stop(t_max_jit_freenect2 *x)
{
    ...
    if (jit_ob) {
        ...
        if (kinect) {
            ...
            post("jit.freenect2: Frame callback disabled.");
        }
    }
}
...
void max_jit_freenect2_assist(t_max_jit_freenect2 *x, void *b, long msg, long arg, char *s){
    if (msg == ASSIST_OUTLET) {
        switch (arg) {
            case 0: sprintf(s, "(matrix) pointcloud"); break;
            case 1: sprintf(s, "(matrix) rgb"); break;
            case 2: sprintf(s, "dumpout"); break;
        }
    }
}
